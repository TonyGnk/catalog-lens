...
intellijPlatform {
    pluginConfiguration {
        ...
        changeNotes = """
            <ul>
                <li>0.8.0: the Releases viewer now knows your catalog version — the matching entry is marked "current", and every other version shows a "Use this version" link that pins it straight into libs.versions.toml (undoable). Works for both GitHub releases and developer.android.com changelogs, which are now split into per-version cards.</li>
                <li>0.7.2: polished the inline release/changelog viewer — theme-aware colors, a normalised heading scale, looser spacing for easier reading, and more padding at the window edges. GitHub release headers (version, name, date) are now selectable so they can be copied. Long lines now soft-wrap to the tool-window width.</li>
                <li>0.7.1: developer.android.com release-notes pages (androidx, AGP, Play Core, …) now render inline in the CatalogLens tool window — the changelog is parsed from the page and shown with formatting and clickable commit/bug links, same as GitHub releases. Pages that can't be parsed fall back to the browser.</li>
                <li>0.7.0: GitHub release notes now open inline. When a [versions] gutter icon resolves to a github.com/owner/repo/releases page, CatalogLens fetches the releases via the GitHub REST API and renders them in a "CatalogLens Releases" tool window — tag, date, and formatted notes with clickable links — instead of opening the browser. Responses are cached for 30 minutes to respect GitHub's unauthenticated rate limit; other links still open in the system browser.</li>
