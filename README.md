...
- **Sort groups A→Z** — a floating editor icon (also in the editor context menu) sorts entries alphabetically within each group. A group is a contiguous run of entry lines; blank lines, comments and table headers act as delimiters and stay in place. A diff preview shows the result before anything is written — each unsorted group gets a checkbox, so you can sort exactly the groups you want, then apply or cancel. Multi-line entries (e.g. `[bundles]` arrays) move as a single unit.
